    ...
    try:
        ...
        for item in candidates:
            ...
            for a in item.find_all("a"):
                ...
                href = a.get("href") or ""
                if is_bad_title(atext):
                    continue
                if atext in ["다운받기", "미리보기", "열기", "펼치기", "접기"]:
                    continue
                if "list.do" in href and atext.lower() in ["list", "목록"]:
                    continue
                title_anchor = a
                break
            ...
            link = urljoin(page_url, title_anchor.get("href") or page_url)
...
def parse_mfds_board(src, start_date, end_date, max_pages=40):
    ...
    for page_no in range(1, int(max_pages) + 1):
        ...
        signature = "|".join([f"{r.get('item_date','')}:{r.get('title','')}" for r in rows[:10]])
    ...
    seen = set()
    for r in all_rows:
        key = item_hash(r.get("site", ""), r.get("category", ""), r.get("item_date", ""), r.get("title", ""), r.get("url", ""))
        if key not in seen:
            seen.add(key)
            deduped.append(r)
...
def collect_mfds_to_db(start_date, end_date, collect_mode="period"):
    ...
    inserted, skipped = db_insert_items(rows)
    return inserted, skipped, len(rows)
